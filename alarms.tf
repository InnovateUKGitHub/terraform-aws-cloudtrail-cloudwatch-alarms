...
locals {
  ...
  sns_topic_arn = length(var.sns_topic_arn) == 0 ? aws_sns_topic.default.arn : var.sns_topic_arn
...
resource "aws_cloudwatch_metric_alarm" "default" {
  ...
  threshold           = local.metric_name[count.index] == "ConsoleSignInFailureCount" || local.metric_name[count.index] == "AuthorizationFailureCount" ? "10" : "3"
...
resource "aws_cloudwatch_dashboard" "main_individual" {
  ...
  dashboard_body = <<-EOF
 {
   "widgets": [
     ${join(",", formatlist(
"{ \"type\":\"metric\", \"x\":%v, \"y\":%v, \"width\":12, \"height\":6, \"properties\":{ \"metrics\":[ [ \"${local.metric_namespace}\", \"%v\" ] ], \"period\":300, \"stat\":\"Sum\", \"region\":\"${var.region}\", \"title\":\"%v\" } } ", local.layout_x, local.layout_y, local.metric_name, local.metric_name))}
   ]
 }
 EOF
}
